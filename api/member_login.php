...
$found = null;
foreach (load_member_users() as $member) {
    if (!is_array($member)) {
        continue;
    }
    if (!empty($member['active']) && auth_lower_text((string)($member['username'] ?? '')) === $username) {
        $found = $member;
        break;
    }
}
...
$_SESSION['member_role_key'] = 'member';

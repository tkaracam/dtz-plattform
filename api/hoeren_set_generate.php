...
$count = (int)($body['count'] ?? 30);
$teil = (int)($body['teil'] ?? 0);
$teil = $teil >= 1 && $teil <= 4 ? $teil : 0;
$includeExplanation = !empty($body['include_explanation']);

try {
    $set = create_training_set('hoeren', $count, $includeExplanation, $teil);
    $downloadName = 'dtz_lid_hoeren_set_' . gmdate('Ymd_His') . '.json';
    ...
    echo json_encode([
        'error' => 'Hören-Set konnte nicht erstellt werden.',
        'detail' => $e->getMessage(),
    ], JSON_UNESCAPED_UNICODE);
}
